...
try:
    from sqlite_backup import get_file_size, verify_sqlite_db, backup_database, restore_database, is_compressed_file
except ImportError:
...
class TestSqliteBackupFunctions:
    ...
    def test_backup_database_uncompressed(self, mock_copy, mock_time):
        ...
        result = backup_database('/src/db.sqlite', '/dest/backup.db', compress=False)
    ...
    def test_backup_database_compressed(self, mock_copyfileobj, mock_open_file, mock_gzip_open, mock_time):
        ...
        result = backup_database('/src/db.sqlite', '/dest/backup.db.gz', compress=True)
    ...
    @patch('builtins.open')
    @patch('shutil.copyfileobj')
    def test_restore_database_from_compressed(self, mock_copyfileobj, mock_open_file, mock_gzip_open, mock_time):
        ...
        result = restore_database('/backup/db.gz', '/restore/db.sqlite', compressed=True)
    ...
    def test_restore_database_uncompressed(self, mock_copy, mock_time):
        ...
        result = restore_database('/backup/db.sqlite', '/restore/db.sqlite', compressed=False)
        
        assert result == 1.5
        mock_copy.assert_called_once_with('/backup/db.sqlite', '/restore/db.sqlite') 